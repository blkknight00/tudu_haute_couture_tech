...
try {
    ...
    $stmt = $pdo->prepare("UPDATE tareas SET estado = 'pendiente' WHERE id = ?");
    $stmt->execute([$tarea_id]);

    if ($stmt->rowCount() > 0) {
        $detalle = $titulo ? "Se restauró la tarea: '$titulo'" : "Se restauró la tarea ID $tarea_id";
        registrarAuditoria($usuario_id, 'restaurar', 'tareas', $tarea_id, $detalle);
        echo json_encode(['status' => 'success']);
    } else {
        echo json_encode(['status' => 'success', 'message' => 'No hubo cambios o tarea no encontrada']);
    }
